...
Shader "Shader Forge/M_WinFX" {
    Properties {
        ...
        _OverallOpacity ("OverallOpacity", Range(0, 1)) = 1
...
////// Emissive:
                float node_3739 = 0.0;
                float4 node_3374 = _Time;
                float2 node_1593 = (i.uv0+node_3374.g*float2(0.07,0.12));
                float4 _TextureDot1_var = tex2D(_TextureDot1,TRANSFORM_TEX(node_1593, _TextureDot1));
                float node_8751 = 0.0;
                float2 node_2669 = (i.uv0+node_3374.g*float2(-0.07,0.12));
                float4 _TextureDot2_var = tex2D(_TextureDot2,TRANSFORM_TEX(node_2669, _TextureDot2));
                float2 node_4372 = (i.uv0+node_3374.g*float2(0.1,0));
                float4 _TextureRay1_var = tex2D(_TextureRay1,TRANSFORM_TEX(node_4372, _TextureRay1));
                float2 node_7971 = (i.uv0+node_3374.g*float2(-0.1,0));
                float4 _TextureRay2_var = tex2D(_TextureRay2,TRANSFORM_TEX(node_7971, _TextureRay2));
                float3 emissive = (((lerp(float3(node_3739,node_3739,node_3739),_ColorDot1.rgb,_TextureDot1_var.b)*_PowerDot1)+(lerp(float3(node_8751,node_8751,node_8751),_ColorDot2.rgb,_TextureDot2_var.b)*_PowerDot2))+lerp((_Color1.rgb+_TextureRay1_var.r),(_Color2.rgb*_TextureRay2_var.r),0.5));
                float3 finalColor = emissive;
                float4 node_3995 = _Time;
                float4 _MaskRay_var = tex2D(_MaskRay,TRANSFORM_TEX(i.uv0, _MaskRay));
                return fixed4(finalColor,(lerp(0.0,lerp(_TextureRay1_var.r,_TextureRay2_var.r,clamp(sin(node_3995.g),0.2,0.8)),_MaskRay_var.g)*_OverallOpacity));
            }
            ENDCG
        }
        Pass {
